...
def _expect_validation_failure(payload: dict[str, object], fragment: str) -> None:
    try:
        validate_v10_data(payload)
    except V10CatalogValidationError as error:
        assert fragment in str(error), str(error)
    else:
        raise AssertionError(f"invalid v10 payload was accepted; expected {fragment!r}")
...
def main() -> None:
    ...
    base["functions"] = [
        ...
        if str(item["function_id"]) not in v11_function_ids | v12_function_ids | v13_function_ids | v14_function_ids | v15_function_ids
    ]
    base["intents"] = [
        item for item in base["intents"]
        if str(item["intent_id"]) not in v11_intent_ids | v12_intent_ids | v13_intent_ids | v14_intent_ids | v15_intent_ids
    ]
    ...
    assert base["catalog_version"] == "9.0.0"
    assert len(base["functions"]) == 1386
    assert len(base["intents"]) == 1252

    stats = validate_v10_data(base)
    assert stats == {
        "functions": 230,
        "terminal_functions": 218,
        "intents": 218,
        "domains": 12,
        ...
        "goal_patterns": 5232,
        "goal_rules": 7848,
        "compositional_goal_rules": 2616,
        "sensitive_reads": 88,
        "state_changing": 130,
        "high_risk": 218,
        "materialized": False,
    }
    assert base == base_snapshot, "validation mutated the v9 base"
    assert REQUIRED_DOMAINS == {
        "property_management_ops",
        "warehouse_fulfillment_ops",
        "maintenance_asset_ops",
        "manufacturing_quality_ops",
        "laboratory_research_ops",
        "classroom_instructor_ops",
        "legal_practice_ops",
        "restaurant_service_ops",
        "family_caregiving",
        "home_energy_management",
        "genealogy_family_history",
        "procurement_supplier_ops",
    }

    function_ids = {str(item["function_id"]) for item in V10_FUNCTIONS}
    terminal_ids = {str(item["function_id"]) for item in V10_FUNCTIONS if item["terminal"]}
    assert REQUIRED_FUNCTIONS <= function_ids
    assert len(V10_FUNCTIONS) == 230
    assert len(terminal_ids) == 218
    assert len(V10_FUNCTIONS) == len(terminal_ids) + len(REQUIRED_DOMAINS)
    assert len(V10_INTENTS) == 218
    assert {str(item["terminal_function"]) for item in V10_INTENTS} == terminal_ids
    assert all(str(item["intent_id"]).startswith("v10_") for item in V10_INTENTS)
    assert all(
        str(rule["rule_kind"]).startswith("v10_")
        and "v10_discriminative_keys" in rule
        and "v9_discriminative_keys" not in rule
        for intent in V10_INTENTS
        for rule in intent["goal_rules"]
    )
    assert all(
        "v10_reviewed_operations" in item["legacy_tags"]
        and "v9_cross_domain" not in item["legacy_tags"]
    ...
    assert len(OFFICIAL_SOURCES) == 32
    ...
    for item in V10_FUNCTIONS:
        ...
        if item["terminal"]:
            sensitive_count += not bool(item["state_changing"])
            state_changing_count += bool(item["state_changing"])
    ...
    assert sensitive_count == 88
    assert state_changing_count == 130
    ...
    for item in V10_INTENTS:
        ...
        assert any(rule["rule_kind"] == "v10_compositional_domain" for rule in item["goal_rules"])
        assert any(rule["rule_kind"] == "v10_consequence_context" for rule in item["goal_rules"])
    ...
    assert merged["catalog_version"] == CATALOG_V10_VERSION
    assert merged["description"] == CATALOG_V10_DESCRIPTION
    assert merged["official_sources_v10"] == OFFICIAL_SOURCES
    assert len(merged["functions"]) == 1616
    assert len(merged["intents"]) == 1470
    validate_catalog_payload(merged)
    assert validate_v10_data(merged)["materialized"] is True
    ...
    with tempfile.TemporaryDirectory(prefix="exitguide-v10-trial-") as temp_dir:
        temp = Path(temp_dir)
        merged_path = temp / "v10-trial.json"
        quality_payload = apply_alias_context_overrides(
            strip_alias_context_overrides(
                merge_v14_with_base(
                    merge_v13_with_base(
                        merge_v12_with_base(merge_v11_with_base(merged))
                    )
                )
            )
        )
        ...
        catalog = NavigationFunctionCatalog(temp / "v10.sqlite", merged_path)
        positive_probes = 0
        for intent in V10_INTENTS:
            for locale in ("ko-KR", "en-US"):
                ...
                positive_probes += 1
        assert positive_probes == 436

        # The development semantic matrix has four probes per intent.  The
        # two contrast probes are derived from reviewed negative role/asset
        # context and must never be normalized into positive matching fields.
        function_by_id = {str(item["function_id"]): item for item in V10_FUNCTIONS}
        contrast_probes = 0
        for intent in V10_INTENTS:
            function = function_by_id[str(intent["terminal_function"])]
            positive_fields = {
                str(value).casefold()
                for value in (
            ...
            negative_context = list(function["negative_context"])
            for probe in (str(negative_context[0]), str(negative_context[3])):
                assert probe.casefold() not in positive_fields
                contrast_probes += 1
        assert contrast_probes == 436
        assert positive_probes + contrast_probes == 872

    partial = copy.deepcopy(base)
    partial["functions"].append(copy.deepcopy(V10_FUNCTIONS[0]))
    _expect_validation_failure(partial, "partial v10")

    changed_function = copy.deepcopy(merged)
    changed_function["functions"][-1]["description"] = "conflicting v10 definition"
    ...
    changed_sources["official_sources_v10"] = {}
    _expect_validation_failure(changed_sources, "official evidence registry")

    changed_metadata = copy.deepcopy(merged)
    changed_metadata["catalog_version"] = "10.0.0-modified"
    _expect_validation_failure(changed_metadata, "materialization metadata")

    unsafe = copy.deepcopy(merged)
    v10_start = len(base["functions"])
    unsafe["functions"][v10_start + 1]["automation_policy"] = "auto_execute"
    _expect_validation_failure(unsafe, "different function or intent definition")

    group_feature_count = sum(len(group.features) for group in GROUPS)
    assert group_feature_count == 218
    print(
        "navigation catalog v10 data checks ok: "
        ...
        f"rules={stats['goal_rules']} sensitive={sensitive_count} "
        f"state_changing={state_changing_count} matrix=872 quality=100 sha256={digest_one}"
    )
