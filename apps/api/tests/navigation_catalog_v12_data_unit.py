...
def _expect_failure(payload: dict[str, object], fragment: str) -> None:
    try:
        validate_v12_data(payload)
    except V12CatalogValidationError as error:
        assert fragment in str(error), str(error)
    else:
        raise AssertionError(f"invalid v12 payload accepted; expected {fragment!r}")
...
def main() -> None:
    ...
    assert SOURCE_DOCUMENT_METADATA == {
        "path": "docs/NAVIGATION_COVERAGE_GAPS_V12.md",
        "algorithm": "sha256",
    ...
    base["functions"] = [
        ...
        if str(item["function_id"]) not in v13_function_ids | v14_function_ids | v15_function_ids
    ]
    base["intents"] = [
        item for item in base["intents"]
        if str(item["intent_id"]) not in v13_intent_ids | v14_intent_ids | v15_intent_ids
    ]
    ...
    assert base["catalog_version"] == "11.0.0"
    assert len(base["functions"]) == 1858
    assert len(base["intents"]) == 1700

    stats = validate_v12_data(base)
    ...
    assert stats["official_sources"] == 74
    assert stats["source_sha256"] == SOURCE_DOCUMENT_SHA256
    assert stats["sensitive_reads"] == 78
    assert stats["state_changing"] == 162
    assert stats["semantic_smoke_probes"] == 1440
    assert stats["collision_probes"] == 384
    ...
    terminal_ids = {str(item["function_id"]) for item in V12_FUNCTIONS if item["terminal"]}
    intent_ids = {str(item["intent_id"]) for item in V12_INTENTS}
    assert len(REQUIRED_DOMAINS) == 12
    assert len(V12_FUNCTIONS) == 252
    assert len(terminal_ids) == 240
    assert len(V12_INTENTS) == 240
    assert {str(item["terminal_function"]) for item in V12_INTENTS} == terminal_ids
    assert all(re.fullmatch(r"[a-z0-9_]+\.[a-z0-9_]+", value) for value in function_ids)
    assert all(re.fullmatch(r"v12_[a-z0-9_]+", value) for value in intent_ids)
    assert all(
        str(rule["rule_kind"]).startswith("v12_")
        and "v12_discriminative_keys" in rule
        and "v11_discriminative_keys" not in rule
        for intent in V12_INTENTS for rule in intent["goal_rules"]
    )
    assert all(
        "v12_governed_operations" in item["legacy_tags"]
        and "v10_reviewed_operations" not in item["legacy_tags"]
        for item in V12_FUNCTIONS
    )

    assert len(OFFICIAL_SOURCES) == 74
    assert len({str(item["url"]) for item in OFFICIAL_SOURCES.values()}) == 74
    ...
    assert all(item["supported_assets"] and item["supported_states"] for item in OFFICIAL_SOURCES.values())
    ...
    assert all(len(values) >= 5 for values in DOMAIN_SOURCE_IDS.values())
    ...
    assert (sensitive, consequential) == (78, 162)

    merged_function_ids: set[str]
    for item in V12_INTENTS:
        assert len(item["patterns_by_locale"]["ko-KR"]) >= 5
        assert len(item["patterns_by_locale"]["en-US"]) >= 5
        assert len(item["goal_rules"]) >= 24
        assert any(rule["rule_kind"] == "v12_compositional_domain" for rule in item["goal_rules"])
        assert any(rule["rule_kind"] == "v12_consequence_context" for rule in item["goal_rules"])
    ...
    assert len(COLLISION_FAMILIES) == 32
    assert len(collisions) == 384
    assert len({item["probe_id"] for item in collisions}) == 384
    ...
    assert merged["catalog_version"] == CATALOG_V12_VERSION
    assert merged["description"] == CATALOG_V12_DESCRIPTION
    assert merged["official_sources_v12"] == OFFICIAL_SOURCES
    assert merged["source_document_v12"] == SOURCE_DOCUMENT_METADATA
    assert len(merged["functions"]) == 2110
    assert len(merged["intents"]) == 1940
    validate_catalog_payload(merged)
    assert validate_v12_data(merged)["materialized"] is True
    ...
    with tempfile.TemporaryDirectory(prefix="exitguide-v12-source-") as temp_dir:
        temp = Path(temp_dir)
        merged_path = temp / "v12-source.json"
        quality_payload = apply_alias_context_overrides(
            strip_alias_context_overrides(
                merge_v14_with_base(merge_v13_with_base(merged))
            )
        )
        quality_payload = apply_alias_context_overrides(
            strip_alias_context_overrides(
        ...
        catalog = NavigationFunctionCatalog(temp / "v12.sqlite", merged_path)
        positive_count = 0
        function_by_id = {str(item["function_id"]): item for item in V12_FUNCTIONS}
        intent_by_function = {str(item["terminal_function"]): item for item in V12_INTENTS}
        for probe in matrix:
        ...
        for probe in collisions:
            ...
            assert plan.terminal_function == probe["expected_function"], probe
            collision_matches += 1
        assert collision_matches == 384

    partial = copy.deepcopy(base)
    partial["functions"].append(copy.deepcopy(V12_FUNCTIONS[0]))
    _expect_failure(partial, "partial v12")

    changed_function = copy.deepcopy(merged)
    changed_function["functions"][-1]["description"] = "conflicting v12 definition"
    ...
    print(
        "navigation catalog v12 source checks ok: "
