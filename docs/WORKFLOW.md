...
- 현재 화면과 경로 단계 매칭
- K-EXAONE 다음 행동 판단
- 메시지형 플로팅 안내, 경로 성공 여부 확인과 안전한 경로 이탈 복구
...
4. K-EXAONE은 `target_element_id`, 경고, 확신도, 현재 경로 상태와 최종 확인 여부를 JSON으로 반환한다.
5. 백엔드는 반환된 요소가 현재 화면에 실제로 존재하고 실패 이력이 없는지 재검증한다.
6. 위험하거나 비가역적인 후보는 자동 실행 대상에서 제외한다.
7. 통과한 결과만 메시지형 플로팅 UI에 전달한다.

### 단계 D — Android 경로 수집과 안내

1. AccessibilityService가 활성 창의 UI 트리를 읽는다.
2. 기록 모드에서 화면 변화와 사용자가 선택한 노드를 저장한다.
3. 플로팅 UI는 `현재 화면에서 ○○을 누르세요`라는 메시지를 표시하고 사용자가 직접 선택한다.
4. 좌표는 해당 순간의 위치 설명이나 선택적 강조에만 사용하고 DB의 주 선택자로 사용하지 않는다.
5. 접근성 정보가 부족한 화면만 별도 비전 분석 대상으로 표시한다.
6. 화면 변경 후 예상 다음 상태인지 자동으로 재검증한다.
7. 해지·삭제·결제 확정 같은 최종 행동은 `requires_user_confirmation=true`로 반환한다.
8. 앱이 자동 클릭하거나 자동으로 뒤로 이동하지 않는다.

### 단계 E — 경로 이탈 복구

1. 예상 화면과 다르면 현재 화면이 다른 알려진 경로 단계인지 재탐색한다.
2. 알려진 단계이면 해당 위치로 경로를 재정렬하고 안내를 계속한다.
3. 알려진 우회 화면이면 검증된 닫기·취소 메뉴를 안내한다.
4. 알 수 없는 화면이면 `recovery.safe=true`인 경우에만 사용자에게 뒤로 가기를 요청한다.
5. 예상 이전 화면 복귀를 AccessibilityService로 확인한다.
6. 실패한 후보를 세션에서 제외하고 안전성이 검증된 대체 후보를 찾는다.
7. 두 후보가 실패하면 추측을 중단하고 `needs_review`로 기록한다.
...
하나의 파일에는 앱·목적·플랫폼·언어·경로 버전 한 조합만 넣는다. 각 단계에는 후보 우선순위, 예상 다음 화면, 방해 요소, 안전한 복귀 방법과 최대 재시도 횟수를 포함한다. Upstage가 자동 청킹하므로 모든 앱 경로를 한 파일에 합치지 않는다.
...
- 현재 화면의 실제 요소 ID만 안내
- 우회·유지·일시정지 버튼 경고
- 사용자가 직접 클릭하고 시스템은 화면 변경만 감지
- 알려진 다른 단계로 경로 재정렬
- 안전한 화면에서만 뒤로 가기 또는 닫기 요청
- 실패 후보 제외 후 검증된 대체 후보 1회 시도
- 후보 2회 실패 시 추측 중단과 경로 업데이트 등록
