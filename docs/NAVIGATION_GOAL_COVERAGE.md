...
| YouTube (collection) | 현재 계정 상태에서 검증 불가(기기 본인 인증 필요) | 목적지 도달 | 재검증 필요(B 고정 이전 A 기록) | 현재 서비스 정책에서 검증 불가(요금제 변경 옵션 없음) | 안전 경계 도달 |
...
- YouTube·제주항공·쿠팡의 2026-08-03 `membership.join` 판정은 B 고정 이전 A 기록이라
  원본은 보존하되 완료 셀에서 제외했다. 세 셀은 공개 Prior가 활성화된 B로 다시 검증한다.
