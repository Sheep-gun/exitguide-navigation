...
ExitGuideLab의 목적 기반 UI 내비게이션 모듈이다. 사용자의 목적과 현재 Android 화면 구조를 받아, 다음에 선택할 UI 요소와 목적을 방해하는 선택지를 안내한다.

## 담당 범위

- 담당: 양건
- 이 저장소: 앱별 모범 경로 수집·검색, 현재 화면 매칭, 다음 행동 판단, AccessibilityService, 플로팅 안내
...
사용자 목적 + 현재 Accessibility 화면 구조
     ...
     검색 결과 + 현재 화면을 K-EXAONE에 전달
                    ↓
 다음 UI 요소·경고·확신도를 구조화된 JSON으로 반환
                    ↓
       Android 플로팅 UI가 사용자에게 안내
```
...
- [ ] Upstage API 키 등록 및 권한 확인
- [ ] `egl-routes-prod`, `egl-sandbox-yanggeon` Vector Store 생성
...
구체적인 작업 순서는 [docs/WORKFLOW.md](docs/WORKFLOW.md)를 따른다.
